...
$$\Delta_m = \text{AIC}_m - \text{AIC}_\text{min}$$

is the difference between the AIC of model $m$ from the minimum AIC among all the candidate models. You can get the model weights with
...
Fill in the blanks: Aside from `ses_cm` and `ses_cmc`, Minorty and Sex are the most important for predicting `mAch`, while sx_cm and minority_cm is the least important?
